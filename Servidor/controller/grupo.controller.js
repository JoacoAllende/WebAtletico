    ...
    mysqlConnection.query("SELECT MAX(grupo) as cantidadGrupos FROM equipo", (err, rows, fields) => {
        if (!err) {
            ...
            for (let i = 1; i <= cantGrupos; i++) {
                const query = 'SELECT * FROM equipo WHERE grupo = ' + i + ' ORDER BY puntos DESC, diferenciaGoles DESC, golesAFavor DESC, golesEnContra DESC';
                mysqlConnection.query(query, (err, rows, fields) => {
                    if (!err) {
                        ...
                        const query = 'SELECT j.*, e1.nombre AS equipoUno, e2.nombre AS equipoDos FROM equipo e1 INNER JOIN juega j ON (j.id_equipoUno = e1.id) INNER JOIN equipo e2 ON j.id_equipoDos = e2.id WHERE j.id_grupo = ' + i + ' ORDER BY j.id_partido';
                        let partidos = [];
                        mysqlConnection.query(query, (err, rows, fields) => {
                            if (!err) {
                                //console.log(rows);
